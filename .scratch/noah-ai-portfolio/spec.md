...
Noah Wang 需要在 Upwork cover letter 中附上一个公开链接，让潜在客户在很短时间内理解他的后端优先定位、相关经历、三个代表性后端与 AI 项目以及下一步如何联系他。仅依赖 Upwork 资料或完整简历，会让客户需要自行筛选大量信息，也不能针对具体项目自然追问。
...
构建一个部署在 Streamlit Community Cloud 的英文后端与 AI 作品集。它在视觉上忠实复刻目标站点的白色极简风格、页面节奏、卡片语言、动画层次和首页到对话的核心交互，同时用 Noah Wang 的真实资料、项目内容和 Upwork 转化路径完全替换目标站点内容。

首页提供无需模型即可浏览的作品面，包括定位、能力、经历、三个项目、合作流程入口和公开联系渠道。快捷卡片使用确定性的站点公开资料；自由提问则由服务端调用 DeepSeek，以自然的第一人称资料口吻回答，并持续明确自己是 AI 生成的作品集而不是 Noah 本人。它不使用 RAG、向量数据库、检索、证据卡片或适配度标签。

主要转化目标是让潜在客户带着目标、当前技术栈和主要阻碍回到 Upwork 与 Noah 沟通。邮箱是辅助联系渠道，GitHub 是项目和源码入口。私有简历、API Key 和其他私人信息不会进入公开仓库、浏览器或生产对话上下文。
...
8. As a 潜在客户, I want 通过 Projects 快捷入口查看三个代表性后端与 AI 项目, so that 我可以聚焦最相关的交付能力
9. As a 潜在客户, I want 通过 Skills 快捷入口浏览分组后的核心技术能力, so that 我能快速核对所需技术栈
10. As a 潜在客户, I want 通过 Experience 快捷入口了解 Noah 的职业经历, so that 我可以理解其后端经验与 AI 集成能力的连续性
...
19. As a 潜在客户, I want 在询问当前合作条件时看到 `$30/hr`、`30+ hrs/week` 和支持 contract-to-hire, so that 我能进行初步合作判断
...
23. As a 潜在客户, I want 了解 Noah 在支付、钱包、区块链、电商和银行后端方面的 7+ 年背景, so that 我可以判断其生产系统经验是否相关
24. As a 潜在客户, I want 了解 Java、Spring、Redis、RabbitMQ、并发和分布式系统是 Noah 的主要工程能力, so that 我能判断其后端交付深度
...
27. As a 潜在客户, I want 看到已确认公开的 Digimart 经历和链接, so that 我能了解数字商品市场、Web3 支付和履约经验
...
30. As a 潜在客户, I want Skills 卡片只展示新版 Upwork 与中文简历共同支持的能力, so that 我不会被夸大的技术清单误导
31. As a 潜在客户, I want Backend、Payments、Integrations、AI Integration、Delivery 和 Business Domains 六组技能按后端优先排列, so that 我能快速扫描 Noah 的能力结构
32. As a 潜在客户, I want 看到 CPcash Wallet 被描述为多链 USDT 钱包与支付后端并链接线上产品, so that 我能理解其支付可靠性经验
33. As a 潜在客户, I want 看到 Digimart 被描述为 Web3 支付与数字商品市场并链接线上产品, so that 我能理解其架构、集成和履约经验
34. As a 潜在客户, I want 看到 Quad Agent 被描述为 production-ready 的 RAG、Deep Research 与 AI Workflow Platform, so that 我能理解它覆盖的产品与代理工作流
35. As a 潜在客户, I want 三个项目卡片整体可点击并在新标签页打开正确目标, so that 我可以直接访问线上产品或公开仓库
36. As a 潜在客户, I want CPcash/Digimart 显示 `Live Product · Visit site` 且 Quad 显示 `Source Available · View on GitHub`, so that 我不会混淆线上产品和源码仓库
37. As a 潜在客户, I want 项目区域明确使用三张 4:3 概念插图, so that 我能快速理解项目主题又不会把生成式视觉误认为真实截图或指标
38. As a 潜在客户, I want 作品集避免 Production Proven、Enterprise Proven 和 Open Source 等未经证实标签, so that 我获得诚实的项目预期
39. As a 潜在客户, I want Quad Agent 的 Production-Ready 表述只表示面向生产部署准备, so that 我不会误认为它已经生产部署或拥有线上指标
...
42. As a 潜在客户, I want 点击顶部 Need backend or AI integration help? 后先进入一条相关对话, so that 我可以在离开站点前理解 Noah 如何帮助我的后端或 AI 集成项目
...
49. As a 潜在客户, I want 桌面和平板宽度下仍能看到 About 入口, so that 精简导航后仍能了解后端与 AI 作品集的边界
...
- 顶部 `Need backend or AI integration help?` 进入对话，并使用与“How can Noah help with my backend or AI integration?”含义一致的初始问题；它不直接把访客送离站点。
- 首页项目区在桌面端显示一行三列的三个项目卡片，并在平板和手机上响应式降为两列和一列。卡片整体可点击并在新标签页打开正确的线上产品或公开仓库。
- 项目区底部提供 `Ask About My Process`，点击后在对话中展示合作流程；首页不增加独立的静态流程区。
- 页面底部保留明确的 Upwork CTA；对话答案可根据语境再次显示 Upwork CTA。
- 首页不增加独立 Experience 时间线。7+ 年经验在 Hero、内容和 Experience 卡片中表达。
- About 使用轻量弹窗，说明后端与 AI 作品集属性、站点公开资料、DeepSeek 生成和本人承诺边界，不展开技术架构论文。
...
- 可公开概述包括：中国、UTC+8、7+ 年工程经验、30+ hrs/week、支持 contract-to-hire。任何城市和费率都不放在静态作品面。
- 当前费率 `$30/hr`、30+ hrs/week 与 contract-to-hire 仅在相关提问中回答；范围、开始时间和最终条款由 Noah 在 Upwork 确认。
...
- 可以公开支付、钱包、区块链、电商、银行后端背景以及 CPcash Wallet、Digimart 和 Quad Agent 名称与公开链接；不出现未确认公开的内部项目名。
...
- Skills 卡片固定按六组后端优先组织：
  ...
  - Full-Stack Delivery：React/TypeScript、全栈 MVP、Docker、CI/CD、自动化测试、部署与交接。
  - Business Domains：Fintech/Payments、Wallets/Web3、Digital Commerce、Supplier/Fulfillment、Banking、E-commerce。
...
- 页面和系统提示持续明确它是后端与 AI 作品集而非 Noah 本人。报价、档期、合同、私人观点和最终承诺必须转交 Noah 并引导至 Upwork。
...
- Upwork CTA 使用 `Need backend or AI integration help?`，提醒潜在客户带上目标、当前技术栈和主要阻碍继续沟通。
...
- 外链测试验证三个项目各自的状态与目标、Upwork、邮箱和 `@noahacgn` 的目标及新标签行为，并验证顶部不出现 Source 入口，但不把第三方页面的可用性作为应用测试成功条件。
...
- 为三个代表项目提供新的托管 Demo；把概念插图称为真实截图、实际订单、精确架构或运行指标；把 Quad Agent 标记为 Production Proven、Enterprise Proven、已生产部署或 Open Source。
...
- 本规格使用领域词汇中的后端与 AI 作品集、作品面、站点公开资料、私有简历、公开事实优先级、第一人称资料口吻、潜在客户、合作条件、合作流程、精选作品、项目概念插图、生产就绪、Upwork 行动入口和公开联系渠道。
...
- CPcash Wallet 和 Digimart 是公开线上产品，但作品集不把可访问页面外推成 Noah 个人完成全部产品或拥有线上指标。
- Quad Agent 是 source-available、经 Noah 确认 production-ready 的应用，但没有许可证或已确认公开生产部署；不要把部署准备等同于生产证明。
